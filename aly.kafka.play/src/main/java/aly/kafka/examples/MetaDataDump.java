...
public class MetaDataDump
{
	...
	{
		kafka.javaapi.consumer.SimpleConsumer consumer = new SimpleConsumer("localhost", 9092,
				100000, 64 * 1024, "aly-id");
...
//		topics2.add("test");
		...
		for (kafka.javaapi.TopicMetadata item : data3)
		{
			...
			for (kafka.javaapi.PartitionMetadata part : item.partitionsMetadata())
			{
				...
				System.out.println("    Partition: " + part.partitionId() + ": Leader: "
						+ part.leader().host() + " Replicas:[" + replicas + "] ISR:[" + isr + "]");
			}
		}
	}
}